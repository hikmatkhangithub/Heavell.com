...
<?php
				$sql=" SELECT brand.id, brand.brandName, category.categoryName,category.id,category.gender, product.id, product.productName, product.productDescription, product.image, product.price, product.instock, product.brandid, product.categoryid FROM category, product, brand WHERE category.id= product.categoryid and category.gender='male' AND brand.id = product.brandid AND brand.id=$id ORDER BY product.id DESC ";
