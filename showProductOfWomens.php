...
<?php
				$sql=" SELECT category.categoryName,category.id, product.id, product.productName, product.productDescription, product.image, product.price, product.instock, product.brandid, product.categoryid,brand.brandName FROM category, product, brand WHERE brand.id=product.brandid and category.id= product.categoryid and category.gender='female' and category.id=$id   ORDER BY product.id DESC ";
